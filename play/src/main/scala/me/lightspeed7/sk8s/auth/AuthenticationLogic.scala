...
trait AuthorizationLogic[TOKEN] {
  ...
  // sync
  // ////////////////////
  def authorizeAnySync[BODY](roles: Set[Role])(action: AuthRequest[AnyContent, TOKEN] => Result)(
      implicit ctx: AuthContext[TOKEN]): Authorized[AnyContent, TOKEN] =
    Authorized(roles, components.parsers.default)(action)

  def authorizeSync[BODY](role: Role)(action: AuthRequest[AnyContent, TOKEN] => Result)(
      implicit ctx: AuthContext[TOKEN]): Authorized[AnyContent, TOKEN] =
  ...
  // async
  // ////////////////////
  def authorizeAny[BODY](roles: Set[Role])(action: AuthRequest[AnyContent, TOKEN] => Future[Result])(
      implicit ctx: AuthContext[TOKEN]): AuthorizedAsync[AnyContent, TOKEN] =
    AuthorizedAsync(roles, components.parsers.default)(action)

  def authorize[BODY](role: Role)(action: AuthRequest[AnyContent, TOKEN] => Future[Result])(
      implicit ctx: AuthContext[TOKEN]): AuthorizedAsync[AnyContent, TOKEN] =
    AuthorizedAsync(Set(role), components.parsers.default)(action)

}

class AuthRequest[BODY, TOKEN](val token: TOKEN, val ctx: AuthContext[TOKEN], request: Request[BODY]) extends WrappedRequest[BODY](request) {
  val received: DateTime = DateTime.now
}

case class AuthorizedAsync[BODY, TOKEN](roles: Set[Role], parser: BodyParser[BODY])(action: AuthRequest[BODY, TOKEN] => Future[Result])(
