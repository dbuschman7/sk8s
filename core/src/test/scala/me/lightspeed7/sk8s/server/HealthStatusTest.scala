...
class HealthStatusTest extends AnyFunSuiteLike with Matchers with BeforeAndAfterAll {
  ...
  test("Health Check") {
    ...
    HealthStatus.summary.toJson.fields.toSet shouldBe Set("sk8s"           -> JsString("health"),
                                                          "overall_health" -> JsBoolean(true),
                                                          "tag1_health"    -> JsBoolean(true),
                                                          "tag2_health"    -> JsBoolean(true)
    ) //  """{"sk8s":"health","overall_health":true,"tag1_health":true,"tag2_health":true}"""
  }
