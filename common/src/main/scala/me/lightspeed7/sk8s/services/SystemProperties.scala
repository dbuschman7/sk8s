...
final class SystemProperties(rawData: String) {
  ...
  def appDependencies(extraSplitKeys: String*)(implicit appInfo: AppInfo): ApplicationDependencies = ApplicationDependencies(appInfo, jarDependencies(extraSplitKeys: _*))
}
...
object JarDependency {
  ...
  def parse(in: String): JarDependency = {
    ...
    def stripVersion(in: String): (String, String) = sVersions.find { v => in.contains(v) } match {
      case None => ("", in)
      case Some(sVer) =>
